...
Verify `.claude/skills/` exists. If not, abort and tell the user to install the skills first from github.com/MichaelvanLaar/github-copilot-config-skills.
...
for skill in copilot-update copilot-init copilot-optimize; do
  ...
  if [ "$skill" = "copilot-update" ] || [ -f "$claude_target" ]; then
    ...
    curl -fsSL "$BASE_URL/.claude/skills/$skill/SKILL.md" -o "$claude_target" \
      && echo "✓ updated $claude_target" || echo "✗ failed $claude_target"
    [ -f "$github_skill" ] && { curl -fsSL "$BASE_URL/.github/skills/$skill/SKILL.md" -o "$github_skill" \
